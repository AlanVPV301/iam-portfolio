...
| --- | --- | --- |
| Purpose | Authentication — who is this user? | Authorization — what can this bearer do? |
| Audience | Your application (`aud` = your client ID) | A resource server (an API) |
| Consumer | Your app validates and reads claims from it | Your app treats it as opaque and forwards it |
| Contains | User identity claims: `sub`, `oid`, `name`, `email` | Scopes, roles, permissions |
...
| HTTPS | HTTP localhost | TLS everywhere — cookies must be `Secure` flag in prod |
...
- Post-logout redirect URI: `http://localhost:5000/` (same origin as the app; register alongside the callback)
